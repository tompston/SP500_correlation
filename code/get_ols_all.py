...
# df = pd.read_csv(r'S&P500_no_outliers.csv')
...
for financial_variable in df.columns:
    if df[financial_variable].dtype == "float64" and financial_variable != "enterprise_value.annual_stock_increase":
        # get 2 variables into a single df, remove a row if observation empty
        current_columns = df[["enterprise_value.annual_stock_increase", financial_variable]].dropna()
...
# save all ols results to csv
# df_all.to_csv("ols_results_outliers.csv")
print(df_all)
